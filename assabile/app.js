...
async function api(path, options = {}) {
  if (state.staticSite && String(path).startsWith("/api/")) {
    throw serverOnlyError(serverFeatureName(path));
  }
  ...
  try {
    ...
    data = { error: response.ok ? text : `${response.status} ${response.statusText}` };
  }
...
function serverFeatureName(path) {
  if (path.includes("/sync/player")) return "Recitation playback";
  if (path.includes("/sync/")) return "Metadata syncing";
  if (path.includes("bulk-download")) return "ZIP downloads";
  if (path.includes("download")) return "Local media caching";
  return "This action";
}

function serverOnlyError(feature = "This action") {
  return new Error(`${feature} needs the local Python server. GitHub Pages is static, so run python server.py locally for caching, syncing, recitation XML resolving, and ZIP downloads.`);
}
...
async function loadStaticCatalog() {
  if (!staticCatalogPromise) {
    staticCatalogPromise = fetch(appAssetUrl("data/catalog.json"), { headers: { Accept: "application/json" } }).then(async (response) => {
      if (!response.ok) throw new Error(`Could not load static catalogue (${response.status})`);
...
async function loadStaticPlayerUrls() {
  if (!staticPlayerUrlsPromise) {
    staticPlayerUrlsPromise = fetch(appAssetUrl("data/player_urls.json"), { headers: { Accept: "application/json" } }).then(async (response) => {
      if (!response.ok) throw new Error(`Could not load static playback index (${response.status})`);
      const payload = await response.json();
      return payload.players || payload;
    });
  }
  const players = await staticPlayerUrlsPromise;
  state.playerUrls = players;
  return players;
}

function playerIdFromXml(playerXml) {
  return String(playerXml || "").match(/onerecitation-(\d+)\.xml/i)?.[1] || "";
}

function staticPlayerMediaUrl(playerXml) {
  return state.playerUrls[playerIdFromXml(playerXml)] || "";
}

async function loadStaticPeoplePayload() {
  const [catalog, playerUrls] = await Promise.all([loadStaticCatalog(), loadStaticPlayerUrls()]);
  return buildStaticPeoplePayload(catalog, playerUrls);
}

async function loadStaticPerson(id) {
  const catalog = await loadStaticCatalog();
  const person = (catalog.people || []).find((item) => item.id === id);
  if (!person) throw new Error("Person not found.");
  return person;
}

function buildStaticPeoplePayload(catalog, playerUrls = {}) {
  const rawPeople = Array.isArray(catalog.people) ? catalog.people : [];
  const countries = new Set();
  const riwayat = new Set();
  const revelations = new Set();
  const surahs = new Set();
  const surahOrder = new Map();
  const tracks = [];

  const rememberSurah = (recitation) => {
    const name = recitation?.surah || "";
    if (!name) return;
    surahs.add(name);
    const number = numeric(recitation.number, 9999);
    if (!surahOrder.has(name) || number < surahOrder.get(name)) {
      surahOrder.set(name, number);
    }
  };
  const sortSurahs = (values) =>
    [...values].filter(Boolean).sort((a, b) => (surahOrder.get(a) || 9999) - (surahOrder.get(b) || 9999) || a.localeCompare(b));
  const addRecordingTracks = (person, groups, kind) => {
    (groups || []).forEach((group) => {
      (group.recordings || []).forEach((recording) => {
        ...
        tracks.push({
          ...
          revelation: "",
          riwayah: "",
          id: mediaUrl || recording.url || "",
          mediaUrl,
          thumb: recording.thumb || "",
          source: recording.source || recording.url || "",
          sourceUrl: recording.source || recording.url || group.url || "",
        });
      });
    });
  };
  ...
  rawPeople.forEach((person) => {
    ...
    (person.collections || []).forEach((collection) => {
      if (collection.riwayah) riwayat.add(collection.riwayah);
    });
    (person.recitations || []).forEach((recitation) => {
      if (recitation.riwayah) riwayat.add(recitation.riwayah);
      if (recitation.revelation) revelations.add(recitation.revelation);
      rememberSurah(recitation);
      tracks.push({
        kind: "recitation",
        personId: person.id,
        personName: person.name,
        title: recitation.surah || "",
        subtitle: recitation.riwayah || "",
        revelation: recitation.revelation || "",
        riwayah: recitation.riwayah || "",
        id: recitation.id || "",
        duration: recitation.duration || "",
        playerXml: recitation.playerXml || "",
        mediaUrl: playerUrls[playerIdFromXml(recitation.playerXml)] || "",
        detailPath: recitation.detailPath || "",
        detailUrl: recitation.detailPath ? `https://www.assabile.com${recitation.detailPath}` : "",
      });
    });
    addRecordingTracks(person, person.albums, "anasheed");
    addRecordingTracks(person, person.audioLessons, "audioLesson");
    addRecordingTracks(person, person.videoLessons, "videoLesson");
    (person.videos || []).forEach((video) => {
      const source = bestVideoSource(video);
      tracks.push({
        kind: "video",
        personId: person.id,
        personName: person.name,
  ...
  const people = rawPeople.map((person) => {
    const personRiwayat = [
      ...(person.recitations || []).map((recitation) => recitation.riwayah),
      ...(person.collections || []).map((collection) => collection.riwayah),
    ];
    return {
      id: person.id,
      name: person.name,
      ...
      tabs: person.tabs || {},
      riwayat: uniqueCanonicalRiwayat(personRiwayat),
      revelations: [...new Set((person.recitations || []).map((recitation) => recitation.revelation).filter(Boolean))].sort(),
      surahs: sortSurahs(new Set((person.recitations || []).map((recitation) => recitation.surah).filter(Boolean))),
    };
  });
  ...
  return {
    ...
    filters: {
      ...
      riwayat: [...riwayat].sort((a, b) => a.localeCompare(b)),
      revelations: [...revelations].sort((a, b) => a.localeCompare(b)),
      surahs: sortSurahs(surahs),
    },
    tracks,
  };
}
...
async function loadDocsPanel() {
  ...
  try {
    if (state.staticSite) {
      const response = await fetch(appAssetUrl("docs/CONTROLS.md"));
      if (!response.ok) throw new Error(`Could not load docs (${response.status})`);
      const body = await response.text();
      panel.innerHTML = `<h2>Controls</h2><pre class="docs-body">${escapeHtml(body)}</pre>`;
    } else {
...
async function bulkDownload(payload) {
  if (state.staticSite) {
    alert(serverOnlyError("ZIP downloads").message);
    return;
  }
...
async function downloadHomePersonImage(button, personId) {
  ...
  if (state.staticSite) {
    if (openExternal(person.image)) button.title = "Opened source image";
    return;
  }
...
function homeTrackQueueItem(track) {
  ...
  if (track.kind === "recitation") {
    return {
      ...
      mediaUrl: track.mediaUrl || staticPlayerMediaUrl(track.playerXml),
...
function homeTrackPlayableQueue() {
  return state.homeMatchedTracks.map(homeTrackQueueItem).filter((item) => item && (state.staticSite ? item.mediaUrl : item.playerXml || item.mediaUrl));
}
...
async function playHomeTrack(index) {
  ...
  if (!selected) return;
  if (state.staticSite && !selected.mediaUrl) {
    alert(serverOnlyError("Recitation playback").message);
    return;
  }
...
function addHomeTrack(button, index) {
  ...
  if (!item) return;
  if (state.staticSite && !item.mediaUrl) {
    alert(serverOnlyError("Recitation playback").message);
    return;
  }
...
async function downloadHomeTrack(button, index) {
  ...
  if (!item) return;
  if (state.staticSite) {
    if (item.mediaUrl && openExternal(item.mediaUrl)) {
      button.title = "Opened source media";
    } else {
      alert(serverOnlyError("Recitation downloads").message);
    }
    return;
  }
  button.disabled = true;
...
async function loadPeople() {
  let data;
  if (state.staticSite) {
    data = await loadStaticPeoplePayload();
  } else {
    try {
...
function recitationQueueItems() {
  return filteredRecitations().map((recitation) => ({
    ...
    id: recitation.id,
    playerXml: recitation.playerXml,
    mediaUrl: recitation.mediaUrl || staticPlayerMediaUrl(recitation.playerXml),
    title: recitationTitle(recitation),
...
async function resolvePlayer(playerXml) {
  ...
  if (state.staticSite) {
    const mediaUrl = staticPlayerMediaUrl(playerXml);
    if (!mediaUrl) throw new Error("This recitation is not available in the static playback index yet. Refresh the Pages playback metadata and redeploy.");
    const data = { source: playerXml, mediaUrl };
...
async function cacheMedia(media) {
  if (state.staticSite) {
    if (!media.mediaUrl) throw serverOnlyError("Local media caching");
    return { ...media, publicPath: media.mediaUrl, bytes: "remote", cacheStatus: "remote" };
  }
  const pathExt = (() => {
    try {
...
async function downloadPlayer(button, playerXml) {
  ...
  try {
    if (state.staticSite) {
      const media = await resolvePlayer(playerXml);
      if (!openExternal(media.mediaUrl)) throw new Error("No media URL is available for this recitation.");
      button.title = "Opened source media";
      return;
    }
...
async function cacheRecording(button, shouldPlay) {
  ...
  try {
    if (state.staticSite && !shouldPlay) {
      ...
      if (!recording.mediaUrl || !openExternal(recording.mediaUrl)) throw serverOnlyError("Local media caching");
      button.title = "Opened source media";
      return;
    }
    if (shouldPlay) {
...
async function downloadProfileVideo(button) {
  ...
  try {
    if (state.staticSite) {
      const source = bestVideoSource(item);
      if (!source?.url || !openExternal(source.url)) throw serverOnlyError("Local media caching");
      button.title = "Opened source video";
      return;
    }
...
async function downloadPhoto(button, url, subdir = `${state.person.id}/photos`, slotIndex = "") {
  if (state.staticSite) {
    if (openExternal(url)) button.title = "Opened source image";
    return;
  }
  button.disabled = true;
  const originalTitle = button.title;
