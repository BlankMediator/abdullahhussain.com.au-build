...
```powershell
python scripts\resolve_player_urls.py
python scripts\build_pages.py --clean
```

The resolver stores direct recitation URLs in `data/player_urls.json` without downloading audio. It is resumable, so reruns keep completed entries and retry only missing player records. The Pages workflow publishes the UI, catalogue, playback index, and this controls document as a static site. Pages can browse/search/filter the catalogue, open source links, and stream indexed recitations or other direct media. Metadata sync, local caching under `data/downloads/`, and ZIP downloads require the local Python server.
